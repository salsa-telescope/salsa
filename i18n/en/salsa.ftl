...
bookings-hours-short = h
bookings-more = more
bookings-show-fewer = Show fewer
...
bookings-help-click = Click a free slot to book it, and one of your own bookings to cancel it — the legend above shows the colour and pattern of each status.
...
observe-js-repeat-next-in = Next in %s s
observe-js-repeat-waiting = Waiting for the next integration
observe-js-repeat-integrating = Integrating
...
vis-intro-1 = For a given target and date, see when it is above the horizon at the SALSA site (Onsala, Sweden) and therefore observable. The chart shows elevation (left axis) and azimuth (right axis, drawn only while the target is up) across the chosen day, with times in
...
vis-date = Date
vis-compute = Compute
vis-hover-hint = Hover or tap the chart to read off time, elevation and azimuth.
vis-lbl-elevation = Elevation
vis-lbl-azimuth = Azimuth
vis-threshold-label = { $threshold }° threshold
