...
bookings-hours-short = tim
bookings-more = till
bookings-show-fewer = Visa färre
...
bookings-help-click = Klicka på en ledig tid för att boka, och på en av dina egna bokningar för att avboka — teckenförklaringen ovan visar färg och mönster för varje status.
...
observe-js-repeat-next-in = Nästa om %s s
observe-js-repeat-waiting = Väntar på nästa integration
observe-js-repeat-integrating = Integrerar
...
observe-error-repeat-too-short = Upprepningsintervallet måste vara minst lika långt som integrationstiden ({ $integration } s).
...
vis-intro-1 = Se när ett givet mål är över horisonten vid SALSA (Onsala) ett valt datum och därmed går att observera. Diagrammet visar elevation (vänster axel) och azimut (höger axel, ritas bara medan målet är uppe) under dygnet, med tider i
...
vis-date = Datum
vis-compute = Beräkna
vis-hover-hint = Håll muspekaren över eller tryck på diagrammet för att läsa av tid, elevation och azimut.
vis-lbl-elevation = Elevation
vis-lbl-azimuth = Azimut
vis-threshold-label = { $threshold }°-gräns
...
login-too-many-starts = För många inloggningsförsök har startats från din adress. Vänta några minuter och försök igen.
