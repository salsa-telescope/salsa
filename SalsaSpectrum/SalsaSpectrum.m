classdef SalsaSpectrum<handle
    ...
    %   control software. The previous +2 offset on CRPIX1 was a workaround
    %   for an off-by-2 quirk in the old GNU Radio FITS writer; the new
    %   writer follows the FITS standard (CRPIX1 = 1 at the first channel),
    %   so the offset is no longer needed and has been removed. Files
    %   produced by the old control software (pre-2026) are no longer
    %   handled correctly by this version — use the version 2.2 file from
    %   the historical varenius/salsa repository for those.
